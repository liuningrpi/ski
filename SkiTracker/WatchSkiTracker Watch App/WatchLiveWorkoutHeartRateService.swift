...
final class WatchLiveWorkoutHeartRateService: NSObject {
    ...
    private static let logger = Logger(subsystem: "com.nliu.SkiTracker.watch", category: "WatchLiveWorkoutHeartRateService")
    ...
    private func beginCollection(builder: HKLiveWorkoutBuilder, startDate: Date) async throws {
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            builder.beginCollection(withStart: startDate) { success, error in
                ...
                guard success else {
                    continuation.resume(throwing: NSError(domain: "WatchLiveWorkoutHeartRateService", code: 1, userInfo: nil))
                    return
                }
